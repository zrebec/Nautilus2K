...
/**
 * Picks the largest integer pixel-scale that lets the 256×192 canvas fit inside
 * the current viewport with a small margin. Capped at 4 (zx-kit reference scale)
 * so we never go bigger than the original ZX Spectrum-style display.
 * Always returns an integer ≥ 1 — no fractional scales (would blur the pixels).
 */
export function pickFittingScale(): number {
  const margin = 60   // body padding + browser chrome breathing room
  const maxByWidth  = Math.floor((window.innerWidth  - margin) / CANVAS_W)
  const maxByHeight = Math.floor((window.innerHeight - margin) / CANVAS_H)
  return Math.max(1, Math.min(maxByWidth, maxByHeight, 4))
}
